...
#include "wifi_credentials.h"   // not tracked in git - only used as fallback if the STM32 doesn't respond
#include "LOGO.h"
...
void setup() {
  ...
  char stm32_ssid[WIFI_PROTO_MAX_SSID + 1];
  char stm32_pass[WIFI_PROTO_MAX_PASS + 1];
  bool configured = false;

  Serial.println("STEP3: before get_wifi_cfg");
  bool got_cfg = wifi_client_get_wifi_cfg(&configured, stm32_ssid, stm32_pass);
  Serial.print("STEP4: get_wifi_cfg returned, got_cfg="); Serial.print(got_cfg);
  Serial.print(" configured="); Serial.println(configured);

  if (got_cfg && configured) {
    Serial.println("WiFi config obtained from the STM32.");
    use_ssid = stm32_ssid;
    use_password = stm32_pass;
  } else {
    Serial.println("No config from the STM32 (or no response) - using wifi_credentials.h.");
  }

  Serial.println("STEP5: before WiFi.begin");
  WiFi.mode(WIFI_STA);
