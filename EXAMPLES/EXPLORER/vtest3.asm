; =============================================================
; VTEST3.ASM -- paso 3 del diagnostico incremental en HW real
;
; Parte de vtest2 (marco estatico confirmado en HW real) y añade la
; PRIMERA comunicacion MCU: OPENDIR2(16) de la raiz + GETROW(18) con
; indice 0 (que devuelve la ruta del directorio actual), convertida de
; codigos ZX81 a ASCII y pintada en la fila 22 (igual que el explorador
; real). Todavia SIN listado de archivos. Se congela en un bucle
; infinito justo despues.
;
; Resultado esperado: marco igual que vtest2, mas la ruta actual (p.ej.
; "/") en la fila 22 con fondo del color de PATH_ATTR, y se queda asi
; PARA SIEMPRE.
;   - Si sale bien -> el protocolo MCU basico (OPENDIR2/GETROW) funciona
;     en HW real; el problema esta mas adelante (el bucle de listado).
;   - Si sale mal / se cuelga -> el problema esta en el protocolo MCU
;     mismo (mcu_send/mcu_recv) o en el propio comando OPENDIR2/GETROW.
;
; Cargar igual que el explorador (mismo stub BASIC, USR 24576). NO vuelve
; nunca al BASIC (hay que apagar/resetear la maquina).
; Ensamblar: pasmo vtest3.asm VTEST3.BIN
; =============================================================
        org 24576
...
PATH_ATTR       equ 020h        ; papel verde, tinta negra (igual que el explorador)
...
start:
        ...
        ; --- primera comunicacion MCU: abrir directorio raiz ---
        call do_opendir_root

        ; --- pedir la fila 0 (ruta actual del directorio) ---
        ld de,0
        call get_row

        ; --- pintarla en la fila 22, con PATH_ATTR, ancho completo ---
        ld a,PATH_ATTR
        ld (cur_attr),a
        ld d,22
        ld e,0
