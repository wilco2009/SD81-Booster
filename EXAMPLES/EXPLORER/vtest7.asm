; =============================================================
; VTEST7.ASM -- paso 7 del diagnostico incremental en HW real
;
; Parte de vtest5 (marco + ruta + listado completo, CONFIRMADO en HW
; real) y añade lo primero que vtest5 no tenia: teclado interactivo real
; (read_key, copiado literal de explorer.asm) con movimiento de cursor
; 6/7 (abajo/arriba) DENTRO de la ventana visible (sin scroll todavia --
; si el movimiento saliera de las 21 filas visibles, simplemente no se
; mueve, a proposito: eso es el siguiente paso). ESPACIO sale al BASIC
; de verdad (video_off + ret), como el explorador real.
;
; Objetivo: aislar si el problema de explorer.asm en HW real (pantalla
; negra con bloque 4 sin remapeo, cuando vtest5 SI funciona ahi) esta en
; la gestion de teclado/redibujado parcial (read_key + redraw_row_attr +
; el bucle principal), no en el bloque de video ni en el listado.
;
; Cargar igual que el explorador (mismo stub BASIC, USR 24576). ESPACIO
; para volver al BASIC.
; Ensamblar: pasmo vtest7.asm VTEST7.BIN
; =============================================================
        org 24576
...
vt_down:
        ...
        jp c,vtd_noscroll
        jp z,vtd_noscroll
        ; se saldria de la ventana visible: paso siguiente de la prueba
        ; incremental (scroll). Por ahora deshacer el avance y no mover.
        ld hl,(cur_index)
        dec hl
        ld (cur_index),hl
        jp vt_loop
...
vt_up:
        ...
        ; se saldria de la ventana visible: deshacer y no mover
        ld hl,(cur_index)
        inc hl
        ld (cur_index),hl
        jp vt_loop
