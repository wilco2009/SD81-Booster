; =============================================================
; VTEST9.ASM -- paso 9 del diagnostico incremental en HW real
;
; Parte de vtest8 (movimiento de cursor 6/7 con scroll, CONFIRMADO en HW
; real) y añade PgUp/PgDn (teclas 1/2, pagina completa) y activar una
; carpeta (8/ENTER: entra y recarga el listado con CD). Todo copiado
; literal de do_pgup/do_pgdn/do_activate/act_dir/strip_brackets/do_cd de
; explorer.asm, con un "vt_refresh" que hace lo mismo que refresh_screen
; completo (video_clear + marco + ruta + listado) para estas operaciones
; que no admiten el repintado parcial de vt_down/vt_up.
;
; Seleccionar un archivo (no carpeta) no hace nada en este test (eso ya
; se probo aparte via el filtrado en BASIC; no es donde se sospecha el
; problema de HW real).
;
; Cargar igual que el explorador (mismo stub BASIC, USR 24576). ESPACIO
; para volver al BASIC.
; Ensamblar: pasmo vtest9.asm VTEST9.BIN
; =============================================================
        org 24576
...
retname:        ; buffer ESTABLE del nombre devuelto a BASIC (24579=ORG+3),
        defs 104 ; igual que en explorer.asm: RAM bloque 3, sobrevive video_off
