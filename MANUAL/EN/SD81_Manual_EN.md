...
| **⚠** | *MC45 mode is implemented by forcing the Z80\'s /HALT pin to zero intermittently and during very brief time intervals, keeping the load on that pin at low levels and making it safe in practice. The interface already includes an internal 680Ω resistor for protection.* |
|------|------------------------------------------------------------------|

## Extending MC45 to blocks 6 and 7

By default, MC45 only covers blocks 4 and 5 (addresses 32768-49151): in native ZX81 video, the screen is generated by jumping to D_FILE+\$8000 and executing NOPs until a real HALT is found (D_FILE itself, in its high mirror), and that address never falls within blocks 4/5 under normal use, so executing real code there is safe.

In the Superfast modes (text, native HiRes or Spectrum HiRes) the video is generated entirely by hardware, without the CPU ever having to "execute" D_FILE --- so, with your own code, it could also be safe to execute in blocks 6 and 7 (49152-65535). The SD81 Booster lets you enable this extension with an independent register:

> POKE 2062, 170 : REM extends MC45 to blocks 6 and 7 as well
>
> POKE 2062, 85 : REM back to the usual restriction (blocks 4 and 5 only)

For this to have any effect, LOAD \*MC45 must also be active; this register only adds blocks 6 and 7 to the area where MC45 already acts, it does not replace it.

| **⚠** | *In the default block mapping (without RAM48 enabled), blocks 6 and 7 are a physical mirror of blocks 2 and 3 --- i.e. of the memory where the system variables, the BASIC program, the stack and the real D_FILE live. Enabling this extension while blocks 6/7 are still that mirror hangs the system as soon as a Superfast mode is enabled, even with FAST. Only enable it after remapping blocks 6 and/or 7 (with MAP) to free SRAM pages dedicated exclusively to your own code.* |
|------|------------------------------------------------------------------|
...
Besides the ZX81\'s built-in BASIC, the SD81 Booster can boot CP/M, the standard operating system of professional 8-bit microcomputers from the late 70s and 80s. Two ports exist for this hardware:

**CP/M 2.2,** the classic version, with no memory banking: up to about 41 KB of TPA (transient program area) in MC45 mode.

**Banked CP/M 3 (CP/M Plus),** which uses the SD81 Booster\'s memory paginator to split RAM into independent 8 KB system and user banks. This is the one documented in this appendix.

CP/M 3 adds, over 2.2: more TPA (the system and the user no longer share the same bank), a RAM disk, the real-time clock integrated into file timestamps, a 256-character colour console, and a network driver for BBS and WiFi communication systems.

The system follows the Digital Research (DRI) standard as far as the hardware allows: same CCP, BDOS and BIOS format, same .COM program conventions, same public BIOS call table. What\'s specific to this port is the memory paging layer (the XIOS) and the device drivers --- disk, keyboard, console, clock and network --- written for the SD81 Booster hardware.

## CP/M 2.2: the two build modes

CP/M 2.2 is distributed in two variants, selected at build time, depending on whether MC45 mode is used (see chapter 11, "Memory management", and in particular "Extending MC45 to blocks 6 and 7"):
...
The 32 KB mode works on any SD81 Booster with nothing special to enable. The 48 KB one needs MC45 turned on, and gives almost double the TPA --- the difference between being able to compile with Turbo Pascal or not.

CP/M 3 needs ALL memory above 32 KB for its bank paging, not just blocks 4 and 5 --- hence it uses the MC45 extension to blocks 6 and 7 described in chapter 11 (POKE 2062,170). CP/M 3\'s own boot process turns it on by itself, with nothing for the user to do.

## Installation and boot

The system is distributed as a single file, SYSTEM.BIN, containing the BIOS, BDOS and CCP already linked together. It loads from BASIC just like any other machine code block and starts with USR:

> LOAD FAST \"SYSTEM.BIN\" CODE 24576
>
> RAND USR 24576

Besides SYSTEM.BIN, the card needs the disk images: A.IMG through D.IMG (256 KB of directory, 2 MB of data each) at the root of the SD card. CP/M 3 images use their own format and are NOT compatible with CP/M 2.2\'s or BASIC\'s --- they can\'t be mixed. A fifth drive, E:, is a RAM disk that needs no image: it\'s created empty on every boot and its contents are lost on power off or reset.

## Banked memory

The SD81 Booster pages memory into 8 blocks of 8 KB. CP/M 3 uses that capacity to keep two complete contexts: a system one (where the BIOS and BDOS live) and a user one (the TPA, where programs run). The topmost block (\$E000-\$FFFF) is common to both contexts --- that\'s where everything that needs to be reachable no matter which one is active lives, including the public BIOS call table (see below).

A user program doesn\'t need to know any of this: the system itself handles the context switch on every BDOS or BIOS call. It only matters if you\'re writing code that accesses memory outside the TPA, or that needs to be faster than a normal BDOS call (see "Calling the BIOS directly").

## Disks: A: to D: (SD card) and E: (RAM)

A: to D: are the four drives backed by the SD card, in standard CP/M 3 format (2 KB allocation block, 256 directory entries). E: is a RAM disk of about 390 KB, fast but volatile --- useful for compiles, temporary files, or anything that doesn\'t need to survive a reset.

MOUNT3.COM lets you hot-swap the image mounted on a drive without restarting the system --- needed to swap virtual floppies during a long session.

## Keyboard

The ZX81 doesn\'t have a full ASCII keyboard: it\'s missing symbols, and only has one arrow key per SHIFT combination. The table below summarises how each character is reached. See "CP/M keyboard table" for the full list.

**Direct and SHIFT+key:** letters, digits and the arrows (WordStar-style: SHIFT+5/6/7/8 = left/down/up/right).

**ENTER+key:** the symbols the ZX81 doesn\'t have printed directly --- including the ones needed to program in C or use a BBS (@ \\ \| \~ \` \_ \# % & !, among others).

**SHIFT+ENTER, then a key:** control mode. Gives \^A-\^Z with letters, and NUL with the space bar.

**SHIFT+9 and SHIFT+0:** backspace (BS, \$08) and delete (DEL, \$7F) --- both, because different remote software expects a different byte for the same function.

**SHIFT+.:** tab (TAB, \$09).

## Real-time clock

CP/M 3 stamps the creation/modification date and time of every file using the board\'s RTC. The clock is set through the SD81 Booster\'s own RTC (for example from BASIC, or by syncing it via NTP if a WiFi module is fitted) --- CP/M only reads whatever the RTC already has.

## Console: 256-character colour terminal

The local console understands a subset of ANSI/VT100 sequences, enough for BBS, full-screen editors and any program that draws in colour:

**Cursor movement:** ESC\[row;colH (or f), ESC\[nA/B/C/D (up/down/right/left), ESC\[s/ESC\[u (save/restore position).

**Clearing:** ESC\[2J (screen), ESC\[K (end of line).

**Colour:** ESC\[\...m (SGR) --- see the SGR utility below for the list of supported codes.

**ESC\[6n (cursor position request):** actually answered, with ESC\[row;colR --- needed because some programs (BBS included) measure the terminal size this way before starting.

256 characters (full CP437 set, not just printable ASCII).

## Calling the BIOS directly

Every BIOS function is reachable from a user program without going through the BDOS, via a public jump table of 3 bytes each, starting at address \$FF00:
...
(address = \$FF00 + index × 3; the full list of 33 entries follows the standard CP/M 3 order).

It\'s the same technique WordStar and Turbo Pascal use to avoid paying the BDOS cost on frequent console operations, and it\'s meant for programs that need the best performance possible --- for example, a communications terminal. The TERM program included with the system (see below) is a real example: it calls CONOUT this way, and that multiplies console speed several times over going through the BDOS.

## Communications

The system includes a network driver (NET) that exposes a raw byte stream to the Z80, as if it had a modem attached --- the software itself decides what to do with that stream (for example, sending AT commands to dial a connection). The real WiFi bridge is handled by the board\'s ESP32 module; the Z80 knows nothing about sockets or addresses.

TERM.COM is the terminal included to use NET. Unlike a typical CP/M program, it does NOT go through the AUX: logical device or the BDOS: it talks to the network hardware directly, with the same direct BIOS call trick described above for screen output. The reason is performance --- a generic terminal honouring DEVICE AUX: would be several times slower --- and it means TERM is built specifically for NET, not as a substitute for a general-purpose communications terminal.
...
SGR parameters sends the ANSI sequence ESC\[parameters m to the console, separated by ;:

| **Code** | **Effect** |
|------------------------------------|------------------------------------|
| **0** | Restore default colours |
| **1** | Bright ink |
...
BORDER n, with n from 0 to 7 (normal colour) or 8 to 15 (same colour, bright, by adding 8) --- see Appendix E, "Chroma81 interface port (7FEFh)", "Writing" section, for the full port format.
...
ENTER+8 and ENTER+9 are labelled by their function (LOG, ECO) rather than by the control code they send (\$1E, \$1C): they\'re the ones TERM uses for its own menu.

| **Key**   | **Direct** | **SHIFT** | **ENTER** |
|-----------|------------|-----------|-----------|
| **1**     | 1          | ESC       | !         |
| **2**     | 2          | ---       | @         |
| **3**     | 3          | ---       | \#        |
...
Also, SHIFT+ENTER then a letter gives \^A-\^Z (letter AND \$1F), and SHIFT+ENTER+space gives NUL.

## CP/M 3 memory map (summary)

| **Range** | **Contents** |
|------------------------------------|------------------------------------|
| **\$0000-\$DFFF** | Banked: TPA (user context) or BIOS/BDOS/CCP (system context), depending on which is active |
| **\$E000-\$FFFF** | Common to both contexts: the SCB (\$FE00), the public BIOS call table (\$FF00), and the rest of the state that must look the same from both sides |

The exact TPA size depends on the final system configuration; it can be checked by booting and looking at CP/M 3\'s startup message (Banked memory, NN.NK TPA).

# Appendix J --- The file explorer

The explorer (EXPLORER.BIN) is an SD card browser in Superfast HiRes Spectrum mode (42 columns, with a compressed 6-pixel font), meant to save having to remember file names or LOAD commands to load a program, view an image, or manage the card.
...
| **Key**       | **Action**                                       |
|---------------|--------------------------------------------------|
| **6**         | Move down the list                               |
| **7**         | Move up the list                                 |
| **1**         | Previous page                                    |
| **2**         | Next page                                        |
| **5**         | Go up to the parent folder                       |
| **8 / ENTER** | Enter the selected folder, or open/load the file |
| **SPACE**     | Exit to BASIC (with no file selected)            |
| **.**         | Filter the listing by wildcards (\* ?)           |
| **SHIFT+1**   | Clear the active filter                          |

The filter persists while you navigate between folders; it\'s only cleared with SHIFT+1.

## What happens when you select a file

Depending on the extension, ENTER/8 does one of these four things without leaving the explorer, or returns the name to BASIC:

| **Extension** | **Behaviour** |
|------------------------------------|------------------------------------|
| **.VGM** | Loaded and starts playing immediately |
| **.PEB** | Same as .VGM, for effects generated with the PEG editor |
| **.SCR** | Shown full-screen (Spectrum capture, 6912 bytes); any key returns to the listing |
| **.TXT** | Opened in the text viewer (see below) |
| **anything else** | The explorer exits to BASIC with the file name, for the stub to decide what to do |

## File operations

| **Key** | **Action** |
|------------------------------------|------------------------------------|
| **N** | New folder (asks for the name) |
| **D** | Delete the selected file or folder (asks for confirmation; folders are only deleted if empty) |
| **R** | Rename (asks for the new name, pre-filled with the current one) |
| **C** | Mark the selected item to copy |
| **X** | Mark the selected item to move |
| **V** | Paste (copies or moves, depending on what was marked with C/X, into the current folder) |

When you mark something with C or X, the matching icon in the bottom bar turns blue as a reminder that something is pending to be pasted.

## Text editing (new folder / rename / filter)

An editor with a real cursor (inserts and deletes at the cursor position, not just at the end):

| **Key**                | **Action**                                       |
|------------------------------------|------------------------------------|
| **letters, digits, .** | Literal character                                |
| **SHIFT+Z/X/C/V**      | : ; ? /                                          |
| **SHIFT+J/K/L**        | \- + =                                           |
| **SHIFT+N/M/.**        | \< \> ,                                          |
| **SHIFT+B**            | \*                                               |
| **SHIFT+SPACE**        | \` (backtick)                                    |
| **SHIFT+5 / SHIFT+8**  | Move the cursor left / right                     |
| **SHIFT+0**            | Delete (the character to the left of the cursor) |
| **SHIFT+1**            | Restore the original name (rename only)          |
| **ENTER**              | Confirm                                          |
| **SPACE**              | Cancel                                           |

## Configuration panel (S)

The S key opens and closes a side panel with the FPGA and music player status, letting you change it without leaving BASIC:

| **Key** | **Action**                                                |
|---------|-----------------------------------------------------------|
| **W**   | Toggle WRX (character generator in the 8-16K RAM)         |
| **F**   | Toggle full paging (FULLPAG)                              |
| **M**   | Toggle MC45                                               |
| **A**   | Toggle the character set between 64 and 128               |
| **J**   | Edit the five joystick keys (up, down, left, right, fire) |
| **T**   | Stop the current music (VGM/PEB)                          |
| **Y**   | Pause it                                                  |
| **U**   | Resume it                                                 |
| **I**   | View /MAN/IP.TXT (the WiFi module\'s IP, if connected)    |

The panel also shows, on its last line, the MCU, ROM and FPGA version (Mx.y,Rx.y,Fx.y). W/F/M/A/J/T/Y/U only act while the panel is visible.

## Text viewer (.TXT)

| **Key**   | **Action**            |
|-----------|-----------------------|
| **6 / 7** | Next / previous line  |
| **1 / 2** | Previous / next page  |
| **SPACE** | Return to the listing |

## Hex viewer

Opened from the configuration panel with the H key, on the file selected in the listing (not available for folders):

| **Key** | **Action** |
|------------------------------------|------------------------------------|
| **6 / 7** | Next / previous row |
| **1 / 2** | Previous / next page |
| **G** | Go to an address (hexadecimal) |
| **Z** | Toggle the right-hand column between ASCII and ZX81 character codes |
| **SPACE** | Return to the listing |

## How a file is returned to BASIC

The explorer lives in RAM as loose machine code (loaded with LOAD \... CODE), so it can\'t touch the BASIC program/variable area itself --- it just returns the name of the selected file (for the types it doesn\'t handle internally) and lets the stub decide. On exit:

1\. It copies the name (already in ASCII) to a fixed address, ORG+3 (24579 with the ORG above), which survives the video restore.

2\. It restores standard video mode and the normal memory mapping (identity: each block to its own numbered page).

3\. It returns the name\'s length in BC (0 if you exited with SPACE) --- in ZX81, USR address as a function returns the contents of BC.
