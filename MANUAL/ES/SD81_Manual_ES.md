...
- [Apéndice H — Protocolo de comunicación MCU: referencia de comandos](#apéndice-h--protocolo-de-comunicación-mcu-referencia-de-comandos)
...
| Registro | Descripción | B7 | B6 | B5 | B4 | B3 | B2 | B1 | B0 |
|----------|-------------|----|----|----|----|----|----|----|----|
| R0  | Canal A — período de tono (byte bajo)  | B7 | B6 | B5 | B4 | B3 | B2 | B1 | B0 |
| R1  | Canal A — período de tono (byte alto)  | — | — | — | — | B3 | B2 | B1 | B0 |
| R2  | Canal B — período de tono (byte bajo)  | B7 | B6 | B5 | B4 | B3 | B2 | B1 | B0 |
| R3  | Canal B — período de tono (byte alto)  | — | — | — | — | B3 | B2 | B1 | B0 |
| R4  | Canal C — período de tono (byte bajo)  | B7 | B6 | B5 | B4 | B3 | B2 | B1 | B0 |
| R5  | Canal C — período de tono (byte alto)  | — | — | — | — | B3 | B2 | B1 | B0 |
| R6  | Período de ruido | — | — | — | B4 | B3 | B2 | B1 | B0 |
| R7  | Habilitación de canales | — | — | Ruido C | Ruido B | Ruido A | Tono C | Tono B | Tono A |
| R8  | Amplitud canal A | — | — | — | Modo env. | L3 | L2 | L1 | L0 |
| R9  | Amplitud canal B | — | — | — | Modo env. | L3 | L2 | L1 | L0 |
| R10 | Amplitud canal C | — | — | — | Modo env. | L3 | L2 | L1 | L0 |
| R11 | Período de envolvente (byte bajo) | B7 | B6 | B5 | B4 | B3 | B2 | B1 | B0 |
| R12 | Período de envolvente (byte alto) | B7 | B6 | B5 | B4 | B3 | B2 | B1 | B0 |
...
|--------|-----------|-------------|
| `61h` | `nn nn` | Esperar n muestras (little-endian, 0–65535; ≈ 1,49 s máximo). Pausas largas se representan con varios comandos de espera consecutivos. |
...
|--------|---------|---------|--------|---------|---------|
| `$00` | PA1 | pausa 10 ms | `$20` | AW | **ou**t |
| `$01` | PA2 | pausa 30 ms | `$21` | DD2 | **d**o |
| `$02` | PA3 | pausa 50 ms | `$22` | GG3 | wi**g** |
| `$03` | PA4 | pausa 100 ms | `$23` | VV | **v**est |
| `$04` | PA5 | pausa 200 ms | `$24` | GG1 | **g**ot |
| `$05` | OY | b**oy** | `$25` | SH | **sh**ip |
| `$06` | AY | sk**y** | `$26` | ZH | a**z**ure |
| `$07` | EH | **e**nd | `$27` | RR2 | b**r**ain |
| `$08` | KK3 | **c**omb | `$28` | FF | **f**ood |
| `$09` | PP | **p**ow | `$29` | KK2 | **sk**y |
| `$0A` | JH | dod**ge** | `$2A` | KK1 | **c**an't |
| `$0B` | NN1 | thi**n** | `$2B` | ZZ | **z**oo |
| `$0C` | IH | s**i**t | `$2C` | NG | a**ng**chor |
| `$0D` | TT2 | **t**o | `$2D` | LL | **l**ake |
| `$0E` | RR1 | **r**ural | `$2E` | WW | **w**ool |
| `$0F` | AX | s**u**cceed | `$2F` | XR | r**epair** |
| `$10` | MM | **m**ilk | `$30` | WH | **wh**ig |
| `$11` | TT1 | par**t** | `$31` | YY1 | **y**es (corto) |
| `$12` | DH1 | **th**ey | `$32` | CH | **ch**urch |
| `$13` | IY | s**ee** | `$33` | ER1 | f**ir** (corto) |
| `$14` | EY | b**ei**ge | `$34` | ER2 | f**ir** (largo) |
| `$15` | DD1 | coul**d** | `$35` | OW | b**eau** |
| `$16` | UW1 | t**oo** | `$36` | DH2 | **th**ey |
| `$17` | AO | **au**ght | `$37` | SS | ve**s**t |
| `$18` | AA | h**o**t | `$38` | NN2 | **n**o |
| `$19` | YY2 | **y**es (largo) | `$39` | HH2 | **h**oe |
| `$1A` | AE | h**a**t | `$3A` | OR | st**ore** |
| `$1B` | HH1 | **h**e | `$3B` | AR | al**ar**m |
| `$1C` | BB1 | **b**usiness (corto) | `$3C` | YR | cl**ear** |
| `$1D` | TH | **th**in | `$3D` | GG2 | **g**uest |
| `$1E` | UH | b**oo**k | `$3E` | EL | sadd**le** |
| `$1F` | UW2 | f**oo**d | `$3F` | BB2 | **b**usiness (largo) |

---

## Apéndice H — Protocolo de comunicación MCU: referencia de comandos

Este apéndice describe el protocolo de bajo nivel entre el Z80 y el MCU. Está dirigido a programadores que deseen comunicarse directamente con el MCU desde código máquina, sin pasar por las rutinas de la ROM extendida.

### Protocolo general

La comunicación se realiza a través del puerto de E/S `A7h`. Cada comando comienza con un byte de código enviado por el Z80, seguido opcionalmente de parámetros. El MCU responde con datos y/o un código de error.

**Formato de cadena Pascal:** `< longitud (1 byte) > < array de hasta 255 bytes >`

Para los detalles de sincronización por bit de reloj, consulta la sección 15.2 de este manual.
...
Z80 → SD81: `01h`  
Z80 ← SD81: `< versión (1 byte, BCD) >`

- Versión mayor = valor / 16
- Versión menor = valor % 16
...
Z80 → SD81: `02h`  
Z80 ← SD81: secuencia de transferencia de cadena. Terminada con `FFh`.
...
Z80 → SD81: `03h, < nombre (cadena Pascal) >`  
Z80 ← SD81: `< código de error >`
...
Z80 → SD81: `04h, < nombre de archivo (cadena Pascal) >`  
Z80 ← SD81: `< código de error >`
...
Z80 → SD81: `05h, < nombre (cadena Pascal) >`  
Z80 ← SD81: `< código de error >`
...
Z80 → SD81: `06h, < nombre (cadena Pascal) >`  
Z80 ← SD81: `< código de error >`
...
Z80 → SD81: `07h, < nombre actual (cadena Pascal) >, < nombre nuevo (cadena Pascal) >`  
Z80 ← SD81: `< código de error >`
...
Z80 → SD81: `08h, < origen (cadena Pascal) >, < destino (cadena Pascal) >`  
Z80 ← SD81: `< código de error >`
...
Carga un archivo. El comportamiento depende de la extensión:

| Extensión | Comportamiento |
|-----------|---------------|
| `.P`, `.81` | Carga programa ZX81. El tamaño se calcula a partir de las variables del sistema en los bytes 11–12 del archivo. |
| `.P81` | Formato multiprograma. Salta la cabecera de nombre antes de leer las variables del sistema. |
| `.ROM` | Carga en dirección 0 y resetea el Z80 y la FPGA. No retorna al BASIC. |
| `.WAV` | Reproduce audio en lugar de cargar datos. |
| *(sin extensión / otras)* | Carga el archivo completo verbatim. |

Z80 → SD81: `09h, < longitud (1 byte) >, < nombre de archivo (string[longitud]) >`  
Z80 ← SD81: `< byte bajo de longitud > < byte alto de longitud > < bytes > < código de error >`
...
Z80 → SD81: `0Ah, < longitud (1 byte) >, < nombre (string[longitud]) >, < byte bajo de longitud de archivo >, < byte alto de longitud de archivo >, < bytes >`  
Z80 ← SD81: `< código de error >`
...
Muestra un archivo de texto en pantalla (implementa `LOAD THEN PRINT`).

- Prefijo `*` en el nombre: busca en `/MAN/<nombre>.TXT` en la SD (sistema de ayuda integrado, similar al comando `man` de Linux).
- Prefijo `?` en el nombre: trata el contenido del archivo como codificación nativa ZX81 (sin conversión).

Z80 → SD81: `0Bh, < longitud (1 byte) >, < nombre (string[longitud]) >`  
Z80 ← SD81: secuencia de transferencia de cadena terminada con `FFh`, seguida de `< código de error >`.

---

### DIR OPEN (0Ch)

Inicia una operación de listado de directorio. Tras este comando, usar GET NEXT CHAR (0Dh) para obtener el listado. Los directorios aparecen entre `<` y `>`.

Z80 → SD81: `0Ch, < directorio + comodines (cadena Pascal) >`  
Z80 ← SD81: `< código de error >`, luego secuencia de transferencia de cadena.

---

### GET NEXT CHAR (0Dh)

Obtiene el siguiente carácter de una secuencia de transferencia de cadena.

Z80 → SD81: `0Dh`  
Z80 ← SD81: `< siguiente carácter >`

`FFh` indica fin de cadena; el byte de estado sigue inmediatamente después de `FFh`. Cualquier comando distinto de `0Dh` termina la secuencia.

---

### FREE TXT (0Eh)

Devuelve el espacio libre en la SD como cadena legible.
...
Devuelve el espacio libre en la SD como valores numéricos crudos de 32 bits.

Z80 → SD81: `0Fh`  
Z80 ← SD81:
```
< tamaño total SD (4 bytes, little-endian) >
< espacio libre SD (4 bytes, little-endian) >
```
...
Construye un array indexado interno de entradas de directorio (hasta 512 entradas). Se usa junto con GETROWLEN y GETROW. El array almacena índices internos de SdFat, no nombres de archivo directamente.

Z80 → SD81: `10h, < directorio + comodines (cadena Pascal) >`  
Z80 ← SD81: `< código de error >`
...
Devuelve la longitud del nombre de archivo de una entrada del array construido por OPENDIR2.

Z80 → SD81: `11h, < índice bajo (1 byte) >, < índice alto (1 byte) >`  
Z80 ← SD81: `< longitud (1 byte) > < código de error >`

---

### GETROW (12h)

Devuelve el nombre de archivo de una entrada del array construido por OPENDIR2.

- Índice 0: devuelve la **ruta del directorio actual** (no una entrada de directorio).
- Otros índices: devuelve el nombre de archivo. Los directorios aparecen entre `<` y `>`.

Z80 → SD81: `12h, < índice bajo (1 byte) >, < índice alto (1 byte) >`  
Z80 ← SD81: `< nombre de archivo (cadena Pascal) >`

---

### ENABLE_M1NOT (13h)

Activa el modo MC45 (permite ejecutar cualquier opcode Z80 en el rango 8000h–BFFFh).
...
Configura el mapeo de teclas del puerto de joystick DB9.

Z80 → SD81: `15h, < 5 bytes en códigos nativos ZX81 >`

Los 5 bytes corresponden a: arriba / abajo / izquierda / derecha / fuego, en ese orden. A diferencia de la mayoría de comandos que reciben cadenas en codificación ZX81 y las convierten internamente a ASCII, estos bytes se usan tal cual (códigos nativos ZX81, no ASCII).
...
Envía un array de bytes de alófonos SP0256 directamente al sintetizador de voz.

Z80 → SD81: `16h, < longitud (1 byte) >, < array de bytes de alófonos >`  
Z80 ← SD81: `< código de error >`

Consulta el Apéndice G para la tabla completa de alófonos.

---

### SAY (17h)

Envía una cadena de texto en inglés al sintetizador de voz. El MCU convierte el texto a fonemas SP0256 internamente mediante un diccionario integrado. Los números se leen en voz alta automáticamente.

Prefijando la cadena con `*` se activa la reproducción en background (el Z80 continúa ejecutándose mientras suena la voz).

Z80 → SD81: `17h, < longitud (1 byte) >, < cadena ASCII >`  
Z80 ← SD81: `< código de error >`
...
Escribe un valor en un registro del chip AY-3-8910/12.

Z80 → SD81: `18h, < registro (1 byte) >, < valor (1 byte) >`  
Z80 ← SD81: `< código de error >`
...
Lee el valor de un registro del chip AY-3-8910/12.

Z80 → SD81: `19h, < registro (1 byte) >`  
Z80 ← SD81: `< valor (1 byte) > < código de error >`

---

### PLAY (1Ah)

Reproduce una cadena de música con el sintetizador AY (comando PLAY).

Z80 → SD81: `1Ah, < longitud (1 byte) >, < cadena de música >`  
Z80 ← SD81: `< código de error >`
...
Activa el modo de 128 caracteres definibles por el usuario. En este modo, la dirección del mapa de bits de caracteres se toma del **registro I del Z80**, tal como en el mecanismo UDG original del ZX81 extendido a 128 caracteres.

Este modo es distinto del modo QuickSilva, que usa una dirección de memoria fija independiente del registro I y solo se activa pulsando el botón **QSILVA** del interface.
...
Vuelve al modo estándar de 64 caracteres definibles.
...
Activa el modo de paginación completa: 512 KB de RAM, 64 páginas de 8 KB.

Este comando lo usa internamente la ROM extendida cuando `LOAD *MAP` se llama con un número de página mayor de 32. No es accesible directamente como comando BASIC independiente, pero puede emitirse desde código máquina.
...
Activa el modo de paginación simple: 256 KB de RAM, 32 páginas de 8 KB.
...
Lee un byte del almacenamiento interno del MCU.

- Índices 0–127: variables volátiles (`sys_vars[]`), se pierden al resetear.
- Índices 128–255: EEPROM (persistente entre resets). El índice 128 corresponde a la posición 0 de la EEPROM.

Z80 → SD81: `20h, < índice (1 byte) >`  
Z80 ← SD81: `< valor (1 byte) >`

---

### SETBYTE (21h)

Escribe un byte en el almacenamiento interno del MCU. El mapeo de índices es el mismo que en GETBYTE.

Z80 → SD81: `21h, < índice (1 byte) >, < valor (1 byte) >`  
Z80 ← SD81: nada
...
Carga un archivo VGM en background y lo prepara para reproducir. Solo se admiten opcodes del chip AY-3-8910/12.

Z80 → SD81: `22h, < longitud (1 byte) >, < nombre de archivo (string[longitud]) >`  
Z80 ← SD81: `< código de error >`
...
Detiene la reproducción VGM y reinicia el emulador AY.
...
Reanuda una reproducción VGM pausada.
...
Establece el modo de bucle del reproductor VGM.

Z80 → SD81: `26h, < modo (1 byte) >`

- `0` = sin bucle (reproduce una vez)
- Cualquier otro valor = bucle continuo
...
Carga bytecode PEG en la memoria del generador de efectos a partir de la dirección indicada.

Z80 → SD81: `28h, < dirección (1 byte) >, < longitud (1 byte) >, < bytecode PEG >`  
Z80 ← SD81: nada
...
Inicia la ejecución de un hilo PEG desde la dirección indicada.

Z80 → SD81: `29h, < hilo (1 byte) >, < dirección (1 byte) >`  
Z80 ← SD81: nada
...
Detiene el hilo PEG indicado.

Z80 → SD81: `2Ah, < hilo (1 byte) >`  
Z80 ← SD81: nada
...
Pausa el hilo PEG indicado.

Z80 → SD81: `2Bh, < hilo (1 byte) >`  
Z80 ← SD81: nada
...
Reanuda un hilo PEG pausado.

Z80 → SD81: `2Ch, < hilo (1 byte) >`  
Z80 ← SD81: nada
...
Carga un archivo binario PEG (`.PEB`) desde la SD en la memoria del generador de efectos.

Z80 → SD81: `2Dh, < longitud (1 byte) >, < nombre de archivo (string[longitud]) >, < dirección (1 byte) >`  
Z80 ← SD81: `< código de error >`

Tamaño máximo del archivo: 512 bytes.

---

### ENABLE_ICHR (2Eh) *(obsoleto)*

Anteriormente activaba el generador interno de caracteres HRG. El cuerpo de la función está completamente comentado en el firmware actual — este comando no hace nada. El modo HRG es ahora transparente y no requiere activación.
...
### DISABLE_ICHR (2Fh) *(obsoleto)*

Anteriormente desactivaba el generador interno de caracteres HRG. Igual que el anterior — no hace nada en el firmware actual.
...
Activa el modo de RAM extendida de 48 KB.
...
Desactiva el modo de RAM extendida de 48 KB, restaurando la compatibilidad estándar.
...
Acceso al reloj en tiempo real.

**Leer fecha/hora actual:**

Z80 → SD81: `32h` (sin parámetros)  
Z80 ← SD81: secuencia de transferencia de cadena con la fecha/hora actual, terminada con `FFh`.

**Ajustar fecha/hora:**

Z80 → SD81: `32h, < longitud (1 byte) >, < cadena de fecha/hora >`

Formatos admitidos:

| Formato | Ejemplo |
|---------|---------|
| `AAAA-MM-DD HH:MM:SS.CC` | `2025-05-04 13:45:00.00` |
| `AAAA-MM-DD HH:MM:SS` | `2025-05-04 13:45:00` |
| `AAAA-MM-DD` | `2025-05-04` |
...
Lee el voltaje de la pila de respaldo del RTC.

Z80 → SD81: `34h`  
Z80 ← SD81: 5 bytes ASCII en formato `V.mmm` (por ejemplo `3.012` para 3,012 V), seguidos de `< código de error >`.
...
| `G` | 1 | Archivo o directorio no encontrado |
| `H` | 2 | No es un directorio |
| `I` | 3 | Error genérico de operación |
| `J` | 4 | El archivo de destino ya existe |
| `K` | 5 | Archivo demasiado grande |
| `L` | 6 | No se puede crear el archivo de destino |
| `M` | 7 | Error de escritura |
| `N` | 8 | Error de lectura |
| `O` | 9 | Valor fuera de rango en un comando PLAY |
| `P` | 10 | Nombre de nota inválido |
| `Q` | 11 | Formato de archivo inválido |
| `R` | 12 | Archivo no abierto |
| `S` | 13 | Operación no permitida (por ejemplo, escritura dentro de un directorio T81) |
